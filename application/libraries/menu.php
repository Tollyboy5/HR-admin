...
class Menu
{
    ...
    public function dynamicMenu()
    {
        ...
        if ($user_type != 1) { // query for employee user role
            $user_menu = $CI->db->query("SELECT tbl_user_role.*,tbl_menu.*
                                        FROM tbl_user_role
                                        INNER JOIN tbl_menu
            ...
            $user_menu = $CI->db->query("SELECT menu_id, label, link, icon, parent FROM  tbl_menu WHERE status=1 ORDER BY sort,time");
        }
